...
class DepartamentosController extends Controller
{
    ...
    public function index() {
        ...
        $botones = array();
        $botones[0] = '<button disabled="disabled" tecla_rapida="F1" style="margin-right: 5px;" class="btn btn-primary btn-sm" id="nuevo-departamento">'.traducir("traductor.nuevo").' [F1]</button>';
        $botones[1] = '<button disabled="disabled" tecla_rapida="F2" style="margin-right: 5px;" class="btn btn-success btn-sm" id="modificar-departamento">'.traducir("traductor.modificar").' [F2]</button>';
        $botones[2] = '<button disabled="disabled" tecla_rapida="F7" style="margin-right: 5px;" class="btn btn-danger btn-sm" id="eliminar-departamento">'.traducir("traductor.eliminar").' [F7]</button>';
    ...
    public function eliminar_departamentos() {
        ...
        try {
            ...
            if(count($miembros) > 0) {
                throw new Exception("NO SE PUEDE ELIMINAR, ESTE DEPARTAMENTO YA ESTA ASIGNADO A UN ASOCIADO");
            }
            ...
            if(count($iglesias) > 0) {
                throw new Exception("NO SE PUEDE ELIMINAR, ESTE DEPARTAMENTO YA ESTA ASIGNADO A UNA IGLESIA");
            }
            ...
            if(count($provincias) > 0) {
                throw new Exception("NO SE PUEDE ELIMINAR, ESTE DEPARTAMENTO YA ESTA ASIGNADO A UNA PROVINCIA");
            }
