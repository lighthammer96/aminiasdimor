...
class BaseModel extends Model
{
    ...
    */
    ...
    public function getPermisos($status = false) {
        ...
        CASE WHEN dt.dt_descripcion IS NULL THEN 'NO TRADUCCION' ELSE dt.dt_descripcion END AS dt_descripcion 
        FROM seguridad.modulos AS m
        LEFT JOIN public.detalle_traducciones AS dt ON ( dt.modulo_id = m.modulo_id AND dt.idioma_id=".session("idioma_id").")
        ...
        if(!$status) {
            ...
            foreach ($modulos as $key => $value) {
                ...
                $sql_hijos = "SELECT * FROM seguridad.modulos WHERE estado='A' AND modulo_padre={$value->modulo_id}
            ...
            foreach ($modulos as $key => $value) {
                ...
                LEFT JOIN public.detalle_traducciones AS dt ON ( dt.modulo_id = m.modulo_id AND dt.idioma_id=".session("idioma_id").")
                WHERE m.estado = 'A' AND p.perfil_id = ".session("perfil_id")."  AND m.modulo_padre = ".$value->modulo_id. "
                GROUP BY m.modulo_id, dt.dt_descripcion
                ORDER BY m.modulo_orden ASC";
                // die($sql_hijos);
