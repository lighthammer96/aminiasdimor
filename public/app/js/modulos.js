...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("ver-modulo").addEventListener("click", function(event) {
        ...
        promise.then(function(response) {
            // modulos.ajax({
            //     url: '/obtener_accionesModulo',
            //     datos: { modulo_id: response.modulo_id }
            // }).then(function(response) {
            //     if(response.length > 0) {
            //         for(let i = 0; i < response.length; i++){
            //             document.getElementById("detalleAcciones").getElementsByTagName("tbody")[0].appendChild(HTMLDetallemodulos(response[i], "disabled"));
            //         }
            //     }
            //     //console.log(response);
            // })
