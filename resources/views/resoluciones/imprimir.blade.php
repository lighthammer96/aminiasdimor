<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Propuesta de Tema</title>
    <style>

        /* referencia: https://ourcodeworld.co/articulos/leer/687/como-configurar-un-encabezado-y-pie-de-pagina-en-dompdf */
        @page {
            margin: 0cm 0cm;
        }

        /** Defina ahora los márgenes reales de cada página en el PDF **/
        body {
            margin-top: 4cm;
            margin-left: 2cm;
            margin-right: 2cm;
            margin-bottom: 2cm;
        }
            
        header {
            position: fixed;
            top: 0.9cm;
            left: 2cm;
            right: 2cm;
            height: 2.5cm;
            text-align: center;
            line-height: 0.8cm;
            font-family: 'Times New Roman' !important;
        }

        * {
            font-family: 'Roboto', sans-serif;
            box-sizing: border-box;
            /* font-weight: bold; */
            font-size: 14px;
        }
		

        /* #contenido {
            
            width: 696px; */
            /* border: 1px solid gray */
					
        /* } */

        /* #logo img {
		
            width: 100%;
        }

        #empresa, #documento {
            padding: 1%;
        }

        #cliente {
            border-radius: 5px; 
            border: 1px solid gray; 
            padding: 1%;
            height: 70px;

        }
     */


        .row {
            width: 100%;
            height: 30px;
            /* margin-top: 15px; */
            /* clear: both; */
        }

        .row-sm {
            width: 100%;
            height: 20px;
            /* margin-top: 15px; */
            /* clear: both; */
        }
        .clear {
            clear: both; 
        }
        .col {
            float: left;
            /* border: 1px solid black; */
        }
        

        h2, h3, h4, h5 {
            /* text-align: center !important; */
            margin: 2px 0;
            /* padding-botton: 2px; */
			
        }
    </style>
   
</head>
<body>
    @include("layouts.cabecera")
    <main>
        
  

        <div class="clear"></div>
        <div class="row"  style="">
            <div class="col" style="width: 100%;">
                <label for=""><strong>{{ $resolucion[0]->resolucion_anio_correlativo }}</strong>&nbsp;&nbsp;&nbsp;{{ $resolucion[0]->tr_titulo_propuesta }}&nbsp;&nbsp;&nbsp;{{ $resolucion[0]->resolucion_fecha }}</label>
            </div>
          
        </div>
        <br>

        <div class="clear"></div>
        <div class="row"  style="">
            <div class="col" style="width: 100%;">
                <label for="">{{ $resolucion[0]->tr_descripcion }}</label>
            </div>
          
        </div>

        <br>

        <div class="clear"></div>
        <div class="row"  style="">
            <div class="col" style="width: 100%;">
                <label for="">{{ traducir("asambleas.propuesta") }}: {{ $resolucion[0]->tr_propuesta }}</label>
            </div>
          
        </div>
        <br>
        <div class="clear"></div>
        <div class="row" style="">
            <div class="col" style="width: 100%;">
                <table border="1" style="width: 100%;">
                    <thead>
                        <tr>
                            <th style="width: 200px;">{{ traducir('asambleas.resultados')}}</th>
                            <th style="width: 50px;">{{ traducir('asambleas.votos')}}</th>
                            <th style="width: 60px;">{{ traducir('asambleas.mano_alzada')}}</th>
                            <th style="width: 50px;">{{ traducir('traductor.total')}}</th>
                            <?php if($resolucion[0]->tabla == "asambleas.propuestas_elecciones") { ?>
                                <th style="width: 50px; " class="ganador">{{ traducir('asambleas.ganador')}}</th>
                            <?php } ?>
                        </tr>

                    </thead>
                    <tbody>
                        <?php 
                        // echo "<pre>";
                        // print_r($resultados); exit;
                            foreach ($resultados as $key => $value) {
                                $ganador = ($value->resultado_ganador == "S") ? "(X)" : "";
                                echo '<tr>';
                                echo '   <td>'.$value->resultado_descripcion.'</td>';
                                echo '   <td>'.$value->resultado_votos.'</td>';
                                echo '   <td>'.$value->resultado_mano_alzada.'</td>';
                                echo '   <td>'.$value->resultado_total.'</td>';
                                if($resolucion[0]->tabla == "asambleas.propuestas_elecciones") {
                                    echo '   <td  align="center">'.$ganador.'</td>';
                                }
                               
                                echo '</tr>';
                            }
                        ?>
                    </tbody>
                    
                </table>
            </div>
        </div> 

    </main>

</body>
</html>